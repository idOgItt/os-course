...
pub fn execve(path: &CStr, argv: &[CString], envp: &[CString]) -> i32 {
    ...
    let mut argv_ptrs: Vec<*const c_char> = Vec::with_capacity(argv.len() + 1);
    for arg in argv {
        argv_ptrs.push(arg.as_ptr());
    }
    argv_ptrs.push(ptr::null());

    let mut envp_ptrs: Vec<*const c_char> = Vec::with_capacity(envp.len() + 1);
    for env in envp {
        envp_ptrs.push(env.as_ptr());
    }
...
pub  fn waitpid(pid: Pid, options: WaitOptions) -> (Pid, WaitStatus) {
    let mut waitpid: WaitStatus = WaitStatus::default();
    let ret:Pid;
    let  ru = 0;
    unsafe {
        asm!(
