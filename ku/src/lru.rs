...
use core::{borrow::Borrow, cmp::Ord, fmt, mem};
    ...
    V: Clone,
{
    ...
    pub fn insert(&mut self, key: K, value: V) -> Option<(K, V)> {
        if let Some(&id) = self.map.get(&key) {
            self.nodes[id].value = value.clone();
            self.move_to_tail(id);
            self.stats.hits += 1;
            None
        } else {
            self.stats.misses += 1;
            let evicted = if self.nodes.len() == self.nodes.capacity() {
                self.evict()
            } else {
                None
            };
            ...
            if let Some(tail_id) = self.tail {
                self.nodes[tail_id].next = Some(new_id);
            }
    ...
    fn evict(&mut self) -> Option<(K, V)> {
        self.head.map(|id| self.remove_node(id)).flatten()
    }
    ...
    fn remove_node(&mut self, id: usize) -> Option<(K, V)> {
        ...
        self.set_next_in_prev(id, node.next);
        self.set_prev_in_next(id, node.prev);
        ...
        if id == self.nodes.len() - 1 {
            ...
            self.nodes.swap(id, last_id);
            self.nodes.pop();

            let swapped_node = self.nodes[id].clone();
            self.map.insert(swapped_node.key.clone(), id);

            if let Some(prev_id) = swapped_node.prev {
                self.nodes[prev_id].next = Some(id);
            } else {
                self.head = Some(id);
            }

            if let Some(next_id) = swapped_node.next {
    ...
    fn set_next_in_prev(&mut self, id: usize, next: Option<usize>) {
        if let Some(prev_id) = self.nodes[id].prev {
    ...
    fn set_prev_in_next(&mut self, id: usize, prev: Option<usize>) {
        if let Some(next_id) = self.nodes[id].next {
    ...
    fn move_to_tail(&mut self, id: usize) {
        ...
        if let Some(tail_id) = self.tail {
            self.nodes[tail_id].next = Some(id);
        }
