...
pub struct GrouppedHex<T: GrouppedHexTrait<T>, const GROUP_HEX_DIGITS: usize = 4>(T);


impl<T: GrouppedHexTrait<T>> GrouppedHex<T> {
...
impl<T: GrouppedHexTrait<T>> fmt::Display for GrouppedHex<T> {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
...
pub trait Tag: Clone + Copy + Default + Eq + Ord {
    ...
    fn is_same_half(x: Addr<Self>, y: Addr<Self>) -> bool;
}
...
impl Tag for PhysTag {
    ...
    fn is_same_half(_x: Phys, _y: Phys) -> bool {
        true
    }
}
...
impl Tag for VirtTag {
    ...
    fn is_same_half(x: Virt, y: Virt) -> bool {
        x.is_higher_half() == y.is_higher_half()
    }
}
...
impl Phys {
    ...
    pub fn new_u32(addr: u32) -> Self {
        ...
        Self(size::from(addr), PhantomData)
    }
}
