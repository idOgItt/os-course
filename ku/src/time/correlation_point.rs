...
impl AtomicCorrelationPoint {
    ...
    pub fn inc(&self, tsc: i64) {
        ...
        self.tsc.store(tsc, Ordering::Release);
    ...
    pub fn store(&self, counter: CorrelationPoint) {
        ...
        self.count.store(counter.count, Ordering::Release);
        self.tsc.store(counter.tsc, Ordering::Release);
    ...
    pub fn load(&self) -> CorrelationPoint {
        loop {
            if let Some(correlation_point) = self.try_load() {
    ...
    fn try_load(&self) -> Option<CorrelationPoint> {
        ...
        let tsc = self.tsc.load(Ordering::Acquire);
