...
use crate::{backtrace::Callsite, log::trace};
...
impl<T, const PANIC_STRATEGY: PanicStrategy> Spinlock<T, PANIC_STRATEGY> {
    ...
    pub fn lock(&self) -> SpinlockGuard<'_, T, PANIC_STRATEGY> {
        ...
        // if let Some(guard) = self.try_lock_impl(100, callsite) {
        //     return guard;
        // }
        //
        // let owner_info =  self.owner.read();
        //
        // panic!(
        //     "Deadlock detected! Spinlock defined at: {}:{}:{}, owner: {:?}, attempted by: {:?}",
        //     self.defined.file(),
        //     self.defined.line(),
        //     self.defined.column(),
        //     owner_info,
    ...
    pub fn try_lock(&self) -> Option<SpinlockGuard<'_, T, PANIC_STRATEGY>> {
        // let callsite = Callsite::new(Location::caller());
        // self.try_lock_impl(100, callsite)
    ...
    //     // //         // TDO: your code here.
    //     // //         PanicStrategy::KnockDown => loop { if let Some(guard) = self.data.try_lock() { return Some(SpinlockGuard { spinlock: guard }); } else { unsafe { self.data.force_unlock(); } } }, // TDO: remove before flight.
    //     // //     }
    //     // // }
    //     //
    ...
    //     //     ).is_ok() {
    //     //         unsafe {
    //     //             let mut guard = self.owner.write();
    //     //
    //     //             guard.set(callsite);
    //     //         }
    ...
    fn try_lock_impl(
        ...
        if panic::is_panicing() {
            match PANIC_STRATEGY {
                PanicStrategy::Halt => unsafe { crate::halt() },
                // TODO: your code here.
                PanicStrategy::KnockDown => loop { if let Some(guard) = self.data.try_lock() { return Some(SpinlockGuard { spinlock: guard }); } else { unsafe { self.data.force_unlock(); } } }, // TODO: remove before flight.
            }
        }
...
impl<T: fmt::Debug> fmt::Debug for Spinlock<T> {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        ...
        //     if let Some(owner) = &self.owner.read() {
        //         write!(formatter, "locked: true, owner: {:?}", owner.location)?;
        //     } else {
        //         write!(formatter, "locked: true")?;
        //     }
        // } else {
        //     let data = unsafe { &*self.data.get() };
        //     write!(formatter, "locked: false, data: {:?}", data)?;
        // }
