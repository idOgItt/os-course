...
use crate::{
    ...
    error::{
        Error::{Elf, InvalidArgument, Overflow},
        Result,
    },
    log::{debug, warn},
...
impl<'a, T: BigAllocator> Loader<'a, T> {
    ...
    ///   [`ElfRange::copy_to_memory()`], см. [`Self::INITIAL_FLAGS`].
    ...
    const INITIAL_FLAGS: PageTableFlags = PageTableFlags::WRITABLE;
}
...
impl TryFrom<ProgramHeader<'_>> for ElfRange {
    ...
    ///
    /// Возвращает ошибку [`Error::Overflow`] если:
