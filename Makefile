...
test: install fs.img
	@( \
	    for crate in $(CRATES); do \
	        cd $$crate; \
	        RUST_BACKTRACE=1 cargo test $$(cargo read-manifest | jq -c ".features | keys" | grep -o '"test"' | sed 's@^"@--features @;s@"@@g') || exit 1; \
	    ...
	    cd ku; \
	    MIRIFLAGS="-Zmiri-disable-isolation" cargo miri test --test 1-time-4-correlation-point; \
	    cd - >/dev/null; \
	    cd sem/rwlock; \
...
doc: FORCE
	@( \
	    ...
	    find target/doc/src -name \*.rs.html -delete; \
