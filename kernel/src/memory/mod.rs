...
///
/// Вызывает [`FrameAllocator::deallocate`] для [frame][Frame] при [`Drop::drop`], если [frame][Frame]
...
impl FrameDeallocatorGuard {
    /// Создает новый [`FrameDeallocatorGuard`] для данного [`Frame`]
    ...
    ///
    /// Возвращает ошибку аллокатора, если аллокация завершилась с ошибкой.
