...
impl Mapping {
    ...
    /// Выделяет с помощью `frame_allocator` фрейм и записывает его
...
pub struct Path<'a> {
    ...
    /// отобращающей виртуальную страницу адреса [`Path::virt`] на его физическую страницу.
...
impl<'a> Path<'a> {
    ...
    /// Выделяет физические фрейы под отсутствующие промежуточные таблицы страничного отображения.
    ///
    /// Возвращает ошибки:
    ///   - [`Error::NoFrame`] если пришлось выделить физический фрейм,
    ///     но их не осталось во `frame_allocator`.
    ///   - [`Error::Unimplemented`] если промежуточный узел таблицы страниц
    ///     имеет флаг [`PageTableFlags::HUGE_PAGE`].
    pub fn map_intermediate(
        &mut self,
        frame_allocator: &mut FrameAllocator,
    ...
    pub(super) unsafe fn unmap(&mut self, frame_allocator: &mut FrameAllocator) -> Result<()> {
        // TODO: your code here.
        unimplemented!();
    }
}
...
pub(super) mod test_scaffolding {
    ...
    pub fn page_directory(mapping: &mut Mapping) -> &mut PageTable {
        unsafe { mapping.page_table_mut(mapping.page_directory()) }
    }
