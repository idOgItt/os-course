...
use ku::{memory::mmu::PageTableEntry, sync::spinlock::Spinlock};
...
impl AddressSpace {
    ...
    pub fn reserve(&mut self, pages: Block<Page>, flags: PageTableFlags) -> Result<()> {
        super::validate_block_flags(pages, flags)?;
...
<<<<<<< HEAD
        if (flags.contains(PageTableFlags::USER_ACCESSIBLE) && !is_user_page(page)) ||
            !(flags.contains(PageTableFlags::USER_ACCESSIBLE) && is_user_page(page))
        {
        ...
        if old_entry.clone()?.present() {
            unsafe {
                path.unmap().expect("Failed to unmap");
            }
...
=======
        ...
        unsafe {
            self.unmap_page(page).ok(); // may not exist
        }

        let virt = page.address();
        let mut path = self.mapping.path(virt);

        path.map_intermediate(flags)?;
        let pte = path.get_mut().expect("Path::map_intermediate should allocate L0 pte");
        pte.set_frame(frame, flags);
>>>>>>> ef04d0b (Done address_space)
...
=======
        let frame = FrameDeallocatorGuard::new(FRAME_ALLOCATOR.lock().allocate()?);
...
>>>>>>> ef04d0b (Done address_space)
    ...
    pub(super) fn remove_excessive_phys2virt(&mut self) -> Result<()> {
        self.mapping.remove_excessive_phys2virt()
    }
}
