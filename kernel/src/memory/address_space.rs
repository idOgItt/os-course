...
use crate::{
    allocator::Big,
    error::{
        Error::{NoPage, PermissionDenied, WrongAlignment},
        Result,
    },
    log::{info, trace, warn},
    process::Pid,
...
impl AddressSpace {
    ...
    pub(super) fn new(page_directory: Frame, phys2virt: Page) -> Self {
        let mapping = Mapping::new(page_directory, phys2virt);
        let page_allocator =
    ...
    pub(crate) const fn zero() -> Self {
        Self {
            ...
            page_allocator: PageAllocator::zero(),
            pid: None,
        }
    }
...
pub(super) mod test_scaffolding {
    ...
    pub fn new_address_space(page_directory: Frame, phys2virt: Page) -> AddressSpace {
        AddressSpace::new(page_directory, phys2virt)
    }
