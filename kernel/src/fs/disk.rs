...
impl Disk {
    ...
    /// Записывает содержимое кеша диска на физический носитель.
    ...
    pub(super) fn pio_read(&self, sectors: Range<usize>, buffer: &mut [u32]) -> Result<()> {
        ...
        unsafe {
            self.send_rw_command(Command::READ, sectors.start, sectors.end - sectors.start)?;
        }
    ...
    pub(super) fn pio_write(&self, sectors: Range<usize>, buffer: &[u32]) -> Result<()> {
        ...
        unsafe {
            self.send_rw_command(Command::WRITE, sectors.start, sectors.end - sectors.start)?;
        }
    ...
    unsafe fn send_command(&self, command: Command, argument: u8) -> Result<()> {
        self.wait_ready()?;
    ...
    unsafe fn write_sector_number(&self, sector: usize) {
        assert!((sector & !SECTOR_NUMBER_MASK) == 0);
...
bitflags! {
    ...
    struct Command: u8 {
        /// Запись содержимого кеша диска на физический носитель.
