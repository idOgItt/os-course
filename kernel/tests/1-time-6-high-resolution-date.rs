...
fn stress() {
    ...
    fn stress<const TICKS_PER_SECOND: i64, const PARTS_PER_SECOND: i64>(
        ...
        debug!(
            ...
            %max_valid_delta,
            %max_delta,
            "expected bounds for the real time delta between two points for the given mock CPU frequency",
        );
...
fn high_resolution_date() {
    ...
    let timer_ticks = TRAP_STATS[Trap::Timer].count();
    let tick_duration = Duration::nanoseconds(NANOSECONDS_PER_TIMER_TICK);
    let max_delta = Duration::nanoseconds(NANOSECONDS_PER_TIMER_TICK * 7 / 5);
    let min_delta = Duration::nanoseconds(NANOSECONDS_PER_TIMER_TICK * 3 / 5);
    ...
    for ticks in 1..TIMER_TICKS {
        while TRAP_STATS[Trap::Timer].count() < timer_ticks + ticks as usize {
            instructions::hlt();
        }
