...
fn t0_path() {
    ...
    let huge_virt = phys2virt(mapping).address();
...
fn t2_map_intermediate() {
    map_intermediate(unique_user_virt());
    do_not_loose_intermediate_flags(unique_user_virt());
}
...
fn t3_no_excessive_intermediate_flags() {
    no_excessive_intermediate_flags(unique_user_virt());
}
...
fn t4_huge_page() {
    ...
    let result = mapping.path(phys2virt.address()).get();
    assert_eq!(result, Err(Unimplemented));

    let result = mapping.path(phys2virt.address()).map_intermediate(USER);
    assert_eq!(result, Err(Unimplemented));
}
...
fn t7_duplicate_drop() {
    ...
    let a = unique_user_virt();
    let b = unique_user_virt();
...
fn check_path(mapping: &mut Mapping, virt: Virt) {
    ...
    for (&child, &parent) in nodes(path).iter().zip(nodes(path)[1..].iter()) {
        if child != Virt::default() {
            ...
            let child_frame_index = (child_page - phys2virt(mapping)).unwrap();
...
fn unique_user_virt() -> Virt {
    const ROOT_LEVEL_ENTRY_SIZE: usize =
        PAGE_TABLE_ENTRY_COUNT.pow(PAGE_TABLE_ROOT_LEVEL) * Page::SIZE;

    static UNIQUE_ROOT_LEVEL_ENTRY: AtomicUsize = AtomicUsize::new(0);

    let offset = UNIQUE_ROOT_LEVEL_ENTRY.fetch_add(1, Ordering::Relaxed) * ROOT_LEVEL_ENTRY_SIZE;
    let virt = (user_pages().start_address() + offset).unwrap();

    debug!(%virt, "allocated unique user virtual address");
