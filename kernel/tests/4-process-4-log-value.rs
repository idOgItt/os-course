...
use ku::{
    error::Error::{InvalidArgument, NoPage, Null, Overflow, PermissionDenied},
    log::{self, Level},
    memory::{mmu::USER_RW, size, Block},
    process::Pid,
...
use kernel::{
    ...
    process::test_scaffolding::{log_value, set_pid},
    Subsystems,
};
...
fn log_value_implementation() {
    ...
    assert!(log_value(process.lock(), info, 1, 0, 0).is_ok());

    assert_eq!(log_value(process.lock(), info, 1, 1, 0), Err(NoPage));

    let result = log_value(process.lock(), info, 0, 0, 0);
    assert!(
        result.is_err(),
    ...
    assert!(
        ...
        "expected Err(InvalidArgument) or Err(Null), got {:?}",
        result,
    );
