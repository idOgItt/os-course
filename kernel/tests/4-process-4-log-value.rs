...
use ku::{
    error::Error::{InvalidArgument, NoPage, Overflow, PermissionDenied},
    log::{self, Level},
...
use kernel::{
    memory::test_scaffolding::{switch_to, user_pages},
...
fn log_value_implementation() {
    ...
    assert!(
        ...
        "expected Err(PermissionDenied) or Err(NoPage), got {:?}",
        result,
    );
