...
fn memory_allocator_alignment() {
    ...
    for lb_align in 0..=max_lb_align {
        ...
        for lb_size in 6..=(max_lb_align + 2) {
            let min_size = (1_usize << lb_size).saturating_sub(100) + 1;
            let max_size = (1 << lb_size) + 100;
...
fn memory_allocator_stress(values: usize) -> usize {
    let max_fragmentation_loss = 4 * MiB;
    let start_info = allocator::info();
    debug!(%start_info);
    ...
    for a in 0..values {
        ...
        my_assert!(current_info_diff.fragmentation_loss() < max_fragmentation_loss);
    }
    ...
    debug!(%end_info_diff);
    my_assert!(end_info_diff.allocations().balance() == 0);
    my_assert!(end_info_diff.fragmentation_loss() < max_fragmentation_loss);
