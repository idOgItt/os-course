...
```rust
{{#include ../../user/cow_fork/src/main.rs:copy_page_table}}
```
...
- К игнорируемым страницам добавляется `trap_stack`, его копировать не нужно. У потомка изначально будет полностью отдельный стек для обработки исключений. Адреса этих стеков предлагается сделать одинаковыми в родителе и потомке --- классический [`fork()`](https://pubs.opengroup.org/onlinepubs/9699919799/functions/fork.html) порождает одинаковые адресные пространства. А отличия в адресных пространствах могут привести к неприятным ошибкам.
