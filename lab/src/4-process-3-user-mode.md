...
- Вспомнить состояние стека ядра. Его можно прочитать по логическому адресу `FS:offset`, где `offset` --- смещение `RSP` ядра в структуре [`Cpu`](../../doc/kernel/smp/cpu/struct.Cpu.html) текущего процессора. Для этого смещения есть константа [`kernel::smp::cpu::KERNEL_RSP_OFFSET_IN_CPU`](../../doc/kernel/smp/cpu/constant.KERNEL_RSP_OFFSET_IN_CPU.html). Не пытайтесь прочитать из `FS` инструкцией [`rdmsr`](https://www.felixcloutier.com/x86/rdmsr). Она испортит регистры пространства пользователя до того как у нас появится место куда их нужно сохранить.
