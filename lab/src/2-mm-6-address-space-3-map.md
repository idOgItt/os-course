...
Если во `flags` включён флаг
[`PageTableFlags::USER_ACCESSIBLE`](../../doc/ku/memory/mmu/struct.PageTableFlags.html#associatedconstant.USER_ACCESSIBLE),
а `page` не принадлежит
[пользовательской части виртуального адресного пространства](../../lab/book/2-mm-6-address-space-1-allocation.html#%D0%93%D1%80%D0%B0%D0%BD%D0%B8%D1%86%D0%B0-%D0%BC%D0%B5%D0%B6%D0%B4%D1%83-%D1%81%D1%82%D1%80%D0%B0%D0%BD%D0%B8%D1%86%D0%B0%D0%BC%D0%B8-%D0%B4%D0%BB%D1%8F-%D1%8F%D0%B4%D1%80%D0%B0-%D0%B8-%D0%B4%D0%BB%D1%8F-%D0%BF%D1%80%D0%BE%D1%81%D1%82%D1%80%D0%B0%D0%BD%D1%81%D1%82%D0%B2%D0%B0-%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D1%82%D0%B5%D0%BB%D1%8F),
возвращается ошибка
[`Error::PermissionDenied`](../../doc/kernel/error/enum.Error.html#variant.PermissionDenied).
Если этот флаг выключен, но `page` принадлежит пользовательской части адресного пространства ---
тоже нужно вернуть ошибку.
Проверить куда попадает `page` можно функцией
[`kernel::memory::is_user_page()`](../../doc/kernel/memory/fn.is_user_page.html).
...
а старый физический фрейм освобождается,
если на него не осталось других ссылок.
Удалить старый маппинг можно с помощью метода
[`Path::unmap()`](../../doc/kernel/memory/mapping/struct.Path.html#method.unmap).
...
А также вспомогательный
[метод](../../doc/kernel/memory/mapping/struct.Path.html#method.unmap)

```rust
unsafe fn Path::unmap(
    &mut self,
) -> Result<()>
```

в файле
[`kernel/src/memory/mapping.rs`](https://gitlab.com/sergey-v-galtsev/nikka-public/-/blob/master/kernel/src/memory/mapping.rs).
Воспользуйтесь
[`Mapping::path()`](../../doc/kernel/memory/mapping/struct.Mapping.html#method.path).
...
```console
 kernel/src/memory/address_space.rs | 57 ++++++++++++++++++++++++++++++++++++++++++++++++---------
