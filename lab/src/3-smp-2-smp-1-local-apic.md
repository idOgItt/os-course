...
В частности, у неё есть такие статические методы:
...
### Задача 3 --- инициализация работы с `LocalApic`
...
Чтобы пользоваться MMIO, нужно:

- Во-первых, замапить его физический адрес в текущее виртуальное адресное пространство.
- А во-вторых, указать процессору, что данный диапазон памяти не нужно кешировать. Для этого требуется выставить флаги [`PageTableFlags::NO_CACHE`](../../doc/ku/memory/mmu/struct.PageTableFlags.html#associatedconstant.NO_CACHE) и [`PageTableFlags::WRITE_THROUGH`](../../doc/ku/memory/mmu/struct.PageTableFlags.html#associatedconstant.WRITE_THROUGH) в [`PageTableEntry`](../../doc/ku/memory/mmu/struct.PageTableEntry.html).
...
в файле
[`kernel/src/smp/local_apic.rs`](https://gitlab.com/sergey-v-galtsev/nikka-public/-/blob/master/kernel/src/smp/local_apic.rs).
...
Запустите тест `3-smp-3-local-apic` из файла
[`kernel/tests/3-smp-3-local-apic.rs`](https://gitlab.com/sergey-v-galtsev/nikka-public/-/blob/master/kernel/tests/3-smp-3-local-apic.rs):

```console
$ (cd kernel; cargo test --test 3-smp-3-local-apic)
...
3_smp_3_local_apic::mapped_properly-------------------------
17:31:59 0 I Local APIC; local_apic = 0v3E_9000; frame = Frame(1043968 @ 0pFEE0_0000); flags = PageTableFlags(PRESENT | WRITABLE | WRITE_THROUGH | NO_CACHE | ACCESSED | DIRTY)
17:31:59 0 I cpu = 0
3_smp_3_local_apic::mapped_properly---------------- [passed]
17:31:59 0 I exit qemu; exit_code = ExitCode(SUCCESS)
```
